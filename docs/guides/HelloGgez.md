...
`ggez` provides an [`EventHandler`](https://docs.rs/ggez/latest/ggez/event/trait.EventHandler.html) as the default recommended interface to its internal loop to use in our games. Thanks `ggez`!

You might have noticed [`EventHandler`](https://docs.rs/ggez/latest/ggez/event/trait.EventHandler.html) is a Rust [Trait](https://doc.rust-lang.org/book/ch10-02-traits.html).
This means it is intended to be implemented on a struct.
There are quite a few callbacks defined on the Trait, but only [2 are required: update and draw](https://docs.rs/ggez/latest/ggez/event/trait.EventHandler.html#required-methods).
...
[`Context`](https://docs.rs/ggez/latest/ggez/context/struct.Context.html) is how `ggez` gives you access to hardware such as mouse, keyboard, timers, graphics, sound, etc..

[`GameResult`](https://docs.rs/ggez/latest/ggez/error/type.GameResult.html) is a utility provided by `ggez` to signify if there was an error or not.
...
To do that, you need to create a [`Context`](https://docs.rs/ggez/latest/ggez/context/struct.Context.html) courtesy of `ggez`.
...
It will also create an [`EventLoop`](https://docs.rs/ggez/latest/ggez/event/struct.EventLoop.html).
We'll need it in a minute to call [`run`](https://docs.rs/ggez/latest/ggez/event/fn.run.html).
...
We'll use [`TimeContext::delta`](https://docs.rs/ggez/latest/ggez/timer/struct.TimeContext.html#method.delta) to get the delta time.
